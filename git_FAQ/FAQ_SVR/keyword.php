...
echo '<div class="Contents2">';
 ...
 // ------------- AND検索 -----------------
		...
		foreach ($kwd as $key => $row) {
			 if ( $sql == "") { 
			 	$sql = " SELECT * FROM $table_list WHERE((quest COLLATE Japanese_CI_AI LIKE '%". $row ."%') 
			 										OR (ans COLLATE Japanese_CI_AI LIKE '%". $row ."%'))"; 
			 }else { 
			 	$sql = $sql."AND ((quest COLLATE Japanese_CI_AI LIKE '%". $row ."%') 
			 					OR (ans COLLATE Japanese_CI_AI LIKE '%". $row ."%'))"; 
			 }
		} 
//  ------------- OR検索 ------------------
			...
			foreach ($kwd as $row) {
				 if ( $sql == "") { 
				 	...
				 	$sql = $sql."OR ((quest COLLATE Japanese_CI_AI LIKE '%". $row ."%') 
				 					OR (ans COLLATE Japanese_CI_AI LIKE '%". $row ."%'))"; 
		 		}
		} 
	}
